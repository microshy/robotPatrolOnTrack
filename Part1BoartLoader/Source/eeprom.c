...
但是可以跨页读。
参考AT24C256的技术手册
改用512k存储的eeprom，取址范围是0x0000到0xFFFF
******************************************************************************/
/*******************************************************************************
0000-F7FF存放待更新的程序
F800-F9FF存放bootloader的中断向量表，FA00-FBFF存放user的中断向量表
FC00-FFFF存放备用数据

FFFD 存放 有完整bootloader的中断向量表标志
FFFE 存放 有完整user的中断向量表标志
FFFF 存放 存在需要更新的软件程序标志
*******************************************************************************/
