...
unsigned char Uart1DataBuffer[256];//数据保存数组
        ...
        case 2:                                                            // Vector 2 - RXIFG
            ...
            case 0x03://数据位和CRC校验位
              ...
              if(Uart1DataBfLength<=Uart1DataNum)//接收预计数量的数据
              {
                ...
                if(crc == Uart1RXBuffer[0])//校验通过
                {
                  Uart1InstructFlag=0x01;
                  Uart1InstructNum=Uart1ExtrInstr(Uart1DataBuffer ,Uart1DataBfLength);
                  if(Uart1InstructNum==0x18)//更新rom
                  {
                      //UCA1IE &= ~UCRXIE  ;
                      
                      updateStatus=0x01;
                      if(Uart1UpdateID==0x01)//主控板
                      {
                          if(Uart1UpdateDataNum==0x00)//第一帧
                          {
                              dataLost=0x00;//
                              UpdateDataNum=Uart1UpdateDataNum;
                              eeprom_write_page(0x0000+(unsigned int)(Uart1UpdateDataNum*250),Uart1DataBuffer+5 ,(unsigned char)(Uart1DataBfLength-5));
                          }
