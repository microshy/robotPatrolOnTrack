...
unsigned char Uart2DataBuffer[256]={0x00};//数据保存数组
int Uart2DataBfLength=0;//数据保存长度
int Uart2DataNum=0;//数据保存计数
extern unsigned char Uart2InstructFlag=0x00;//接收到指令标志
unsigned char Uart2InstructSource=0x00;//指令发送源地址
unsigned char Uart2InstructAim=0x00;//指令发送目的地址
extern unsigned char Uart2InstructNum=0x00;//指令编号

extern unsigned char Uart2UpdateID=0x00;//rom更新设备ID
extern unsigned char Uart2UpdateDataNum=0x00;//rom更新数据序号
unsigned char UpdateDataNum;
extern unsigned char Uart2UpdateStatus=0x00;//rom更新状态位
extern unsigned int addrEeprom=0x0000;//
extern unsigned char updateState=0x00;//软件更新状态
unsigned char dataLost=0x00;

extern unsigned char moveDir=0x00;//0x00表示停止，0x01表示前，0x02表示后，0x03表示下
extern unsigned char Uart2moveRes[3]={0x00};//主控板移动电动推杆指令
extern unsigned char Uart2movetoRes[2]={0x00};//主控板电动推杆移动到指令
extern unsigned char Uart2ptzRollRes[3]={0x00};//主控板移动云台指令
extern unsigned char Uart2ptzRollToRes[4]={0x00};//主控板云台移动到指令
//****************************************************************************
//                                                                           *
//                       串口初始化                                          *
//                                                                           *
...
void Init_UART2(void)
{
    ...
    P9SEL |= TXD485BUS  + RXD485BUS                               ;  //引脚功能选择
    P9DIR |= TXD485BUS                                           ;  //设置方向输出
    P9DIR &= ~ RXD485BUS                                         ;  //设置方向输入
...
__interrupt void USCI_A1_ISR(void)
{
    switch(__even_in_range(UCA1IV,4))
    {
        ...
        case 2:                                                            // Vector 2 - RXIFG
            ...
            {
            case 0x00:
              if(Uart1RXBuffer[0] == 0xFF)//数据头
              {
                  Uart1RXFlag = 0x01;//找到数据头
              }
              break;
            case 0x01:
              distData[0] = Uart1RXBuffer[0];//距离数据高位
              Uart1RXFlag = 0x02;
              break;
            case 0x02:
              distData[1] = Uart1RXBuffer[0];//距离数据低位
              Uart1RXFlag = 0x03;
              break;
            case 0x03:
              if(((distData[0]+distData[1])&0xFF) == Uart1RXBuffer[0])//校验位
              {
    ...
    switch(__even_in_range(UCA2IV,4))
    {
        ...
        case 2:   // Vector 2 - RXIFG
            ...
            case 0x01://数据长度位
              ...
              Uart2RXFlag=0x02;//找到数据长度位
              break;
            case 0x02://目的地址
              Uart2DataNum++;//用于计数
              Uart2DataBuffer[Uart2DataNum]=Uart2RXBuffer[0];//保存目的地址
              Uart2InstructAim=Uart2RXBuffer[0];
              Uart2RXFlag=0x03;//找到数据长度位
              break;
            case 0x03://源地址
              Uart2DataNum++;//用于计数
              Uart2DataBuffer[Uart2DataNum]=Uart2RXBuffer[0];//保存源地址
              Uart2InstructSource=Uart2RXBuffer[0];
              Uart2RXFlag=0x04;//找到数据长度位
              break;
            case 0x04://指令编号
              Uart2DataNum++;//用于计数
              Uart2DataBuffer[Uart2DataNum]=Uart2RXBuffer[0];//保存指令编号
              Uart2InstructNum=Uart2RXBuffer[0];
              Uart2RXFlag=0x05;//找到数据长度位
              break;
            case 0x05://数据位和CRC位
              Uart2DataNum++;//用于计数
              Uart2DataBuffer[Uart2DataNum]=Uart2RXBuffer[0];//数据位
              if(Uart2DataBfLength<=Uart2DataNum)//接收预计数量的数据
              {
                unsigned char crc=Uart2RXBuffer[0];//保存CRC位
                if((Uart2InstructAim==0x03) &&(crc == CRC(Uart2DataBuffer,Uart2DataBfLength)))//目的地址正确，校验正确
                {
                  Uart2InstructFlag=0x01;
                  Uart2InstructNum=Uart2ExtrInstr(Uart2DataBuffer ,Uart2DataBfLength);
                  
                  if(Uart2InstructNum==0x18)//rom更新
                  {
                      if(Uart2UpdateDataNum==0x00)//第一帧
                      {
                          updateState=0x01;
                          dataLost=0x00;//
                      ...
                      if(Uart2UpdateDataNum!=0x00)//不是第一帧
                      {
                          if(UpdateDataNum+1==Uart2UpdateDataNum)//连续帧
                          {
                              UpdateDataNum=Uart2UpdateDataNum;
                              addrEeprom=eeprom_write_page(0x0000+(unsigned int)(Uart2UpdateDataNum*249),Uart2DataBuffer+6 ,(unsigned char)(Uart2DataBfLength-6));
                          }
                          else if(UpdateDataNum==Uart2UpdateDataNum)//重复帧
                          {}
                          else
                          {
                            dataLost=0x01;//数据丢失
                      ...
                      if(Uart2UpdateStatus==0x01)//最后一帧
                      {
                          if(dataLost==0x00)
                          {
                              delay_ms(5);
...
unsigned char Uart2ExtrInstr(unsigned char *buffer ,int length)
{
    switch (buffer[3])//指令编号
    {
        ...
        case 0x03://电动推杆停止运动指令
          
            return 0x03;

        case 0x04://电动推杆移动到指定位置指令
            for(int i=0;i<3;i++)
                Uart2movetoRes[i]=buffer[4+i];
            return 0x04;

        case 0x05://电动推杆移动到位置指令，清除变量指令
        ...
        case 0x18:
          ...
          Uart2UpdateDataNum=buffer[4];//数据序号
          Uart2UpdateStatus=buffer[5];//状态位
