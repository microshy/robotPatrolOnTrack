...
extern unsigned char chrgstateOld;
        ...
        case 2:   // Vector 2 - RXIFG
            ...
            case 0x05://数据位和CRC位
              ...
              if(Uart1DataBfLength<=Uart1DataNum)//接收预计数量的数据
              {
                ...
                if((Uart1InstructAim==0x05) &&(crc == CRC(Uart1DataBuffer,Uart1DataBfLength)))//目的地址正确，校验正确
                {
...
unsigned char Uart1ExtrInstr(unsigned char *buffer ,int length)
{
    switch (buffer[3])
    {
        case 0x01://状态请求指令
        {
          chrgstateOld=chrgstateNow;
          chrgstateNow=buffer[4];
            return 0x01;
        }
        case 0x02://自检请求指令
            return 0x02;
